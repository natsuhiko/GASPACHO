...
Verbose=0){
    ...
    PM = diag(length(M))[rep(1:length(M),M),]
    # PM = PM[PM[,2]>0,] # debug for single GPs
    ...
    for(i in 1:length(Ta)){
        ...
        if(i==1){ # Periodic
            ...
            gradTa = c(gradTa, list(matrix(-colSums(tC*dqKX) + rowSums(2*tAA*dqKT)/2, M[i])))
        }else{ # SE
    ...
    if(is.null(Param$GradRhoXi)){
        print("init LBFGS")
        #ValNew  = matrix(c(2*log(rho[[1]]), c(Xi[[1]], c(Ta[[1]])), c(rbind(t(2*log(rho[[2]])),Xi[[2]],Ta[[2]]))), 1+N+M[1]+(1+N+M[2])*Q)
        #GradNew = matrix(c(gradRho[[1]], gradXi[[1]], gradTa[[1]], c(gk)),1+N+M[1]+(1+N+M[2])*Q)
        #ValNew  = matrix(c(2*log(rho[[1]]), c(rbind(t(2*log(rho[[2]])),Xi[[2]],Ta[[2]]))), 1+(1+N+M[2])*Q)
        #GradNew = matrix(c(gradRho[[1]], c(gk)),1+(1+N+M[2])*Q)
        
        ss = -c(GradNew)/5000
    }else{
        #ValNew  = cbind(c(2*log(rho[[1]]), c(Xi[[1]]), c(Ta[[1]]), c(rbind(t(2*log(rho[[2]])),Xi[[2]],Ta[[2]]))), Param$ValRhoXi)
        #GradNew = cbind(c(gradRho[[1]], gradXi[[1]], gradTa[[1]],  c(gk)),Param$GradRhoXi)
        #ValNew  = cbind(c(2*log(rho[[1]]), c(rbind(t(2*log(rho[[2]])),Xi[[2]],Ta[[2]]))), Param$ValRhoXi)
        #GradNew = cbind(c(gradRho[[1]],  c(gk)),Param$GradRhoXi)
        ValNew = cbind(ValNew,Param$ValRhoXi)
        GradNew = cbind(GradNew,Param$GradRhoXi)
        ss = -LBFGS(ValNew, GradNew)
    }
    #ss = list(ss[1:(1+N+M[1])], matrix(ss[-seq(1+N+M[1])], 1+N+M[2]))
    #ss = list(ss[1], matrix(ss[-1], 1+N+M[2]))
    ss2 = list()
    for(i in 1:length(M)){
        ss2 = c(ss2, list(matrix(ss[(strend[i]+1):strend[i+1]],1+N+M[i])))
    }
    ss = ss2
    
    tLD = dbind(Param$LD, forwardsolve(t(chol(K)),diag(sum(M)))*sqrt(Theta))
    pelbonow = sum(c(- logDet(diag(K2+sum(M))+t(tLD)%*%(t(tZ/omega2)%*%tZ)%*%tLD), + sum(diag(Solve(Phiinv,G%*%t(t(D)-E%*%tA)/J))),
                + sum(t(Knm/omega2)*Solve(K/Theta,t(Knm))), - sum(Xi[[2]]^2)/J - sum(Ta[[2]]^2)/J - sum(Xi[[3]]^2)/J - sum(Ta[[3]]^2)/J)) #- sum(1/omega2)*theta 
    pelbo0=pelbo1=-(10^10)
    rho0 = rho1 = rho
    Xi0  = Xi1  = Xi
    Ta0  = Ta1  = Ta
    ...
    if(0){ # reverse
        ...
        for(r in c(0:10,20,50)){
            ...
            #Ta1  = list(Ta[[1]]           -  ss[[1]][(N+2):(1+N+M[1])]/2^r, Ta[[2]]           - ss[[2]][(N+2):(1+N+M[2]),]/2^r)
            #rho1 = list(exp(log(rho[[1]]) - (ss[[1]][1]/2.)/2^r),         exp(log(rho[[2]]) - (ss[[2]][1,]/2.)/2^r))
            #Xi1  = list(Xi[[1]]           ,        Xi[[2]]           - ss[[2]][2:(1+N),]/2^r)
            #Ta1  = list(Ta[[1]]           , Ta[[2]]           - ss[[2]][(N+2):(1+N+M[2]),]/2^r)
            for(i in 1:length(M)){
                rho1[[i]] = exp(log(rho[[i]]) - (ss[[i]][1,]/2.)/2^r)
                Xi1[[i]]  = Xi[[i]]           -  ss[[i]][2:(1+N),]/2^r
                Ta1[[i]]  = Ta[[i]]           -  ss[[i]][(N+2):(1+N+M[i]),]/2^r
            }
            if(Verbose>1){cat("rho1=");print(unlist(rho1))}
            pelbo1 = getELBOrho(Yt, YMat, Data, Param, Xi1, Ta1, rho1)
            if(Verbose>1){print(c(as.integer(r),pelbo0,pelbo1))}
