...
Verbose=0){
    ...
    PM = diag(length(Ta))[rep(1:length(Ta),unlist(lapply(Ta,nrow))),]
    ...
    tC = (C*Knm)[,PM[,2]>0]
    tAA = (AA*K)[PM[,2]>0,PM[,2]>0]
    # Xi
    gradXi = list(rowSums((C*Knm)[,PM[,1]>0]*(2*cos(-outer(c(Xi[[1]]),c(Ta[[1]]),"-"))*sin(-outer(c(Xi[[1]]),c(Ta[[1]]),"-"))))/rho[[1]]^2,
        (- rowSums(tC)*Xi[[2]] + tC%*%Ta[[2]])%*%diag(1/rho[[2]]^2) - Xi[[2]]/J)
    
    # rho
    dqK   = sin(outer(c(Ta[[1]]),c(Ta[[1]]),"-"))^2/rho[[1]]^2. # K  *
    dqKnm = sin(outer(c(Xi[[1]]),c(Ta[[1]]),"-"))^2/rho[[1]]^2. # Knm*
    gradRho = list(sum((C*Knm)[,PM[,1]>0]*dqKnm) + sum((AA*K)[PM[,1]>0,PM[,1]>0]*dqK)/2, rep(0,Q))
    for(q in seq(Q)){
        dqK   = outer(Ta[[2]][,q],Ta[[2]][,q],"-")^2/rho[[2]][q]^2/2. # K  *
        dqKnm = outer(Xi[[2]][,q],Ta[[2]][,q],"-")^2/rho[[2]][q]^2/2. # Knm*
        gradRho[[2]][q] = sum(tC*dqKnm) + sum(tAA*dqK)/2
    }
    
    # Ta
    gradTa = (t(tC)%*%Xi[[2]] - colSums(tC)*Ta[[2]])%*%diag(1/rho[[2]]^2) - Ta[[2]]/J + (tAA-diag(rowSums(tAA)))%*%Ta[[2]]%*%diag(1/rho[[2]]^2)
