...
W0
    ...
    Param1=gplvm$Param
    Param1$Xi[[2]] =Param1$Xi[[2]][,1,drop=F]
    Param1$Ta[[2]] =Param1$Ta[[2]][,1,drop=F]
    Param1$Ta = reducedDim(Param1$Ta)
    Param1$rho[[2]]=Param1$rho[[2]][1]
    ...
    L  = dbind(gplvm$Param$LD, backsolve(chol(gplvm$Param$K),diag(M)) * sqrt(theta))
    ...
    # immune clustering
    Ks2 = updateKernelSE(gplvm$Param$Xi[[2]][,-1], gplvm$Param$Ta[[2]][,-1], gplvm$Param$rho[[2]][-1])
...
rhoscale=1.0
    ...
    L  = dbind(gplvm$Param$LD, backsolve(chol(gplvm$Param$K),diag(M)) * sqrt(gplvm$Param$theta), backsolve(chol(Ks$K),diag(M))*sqrt(gplvm$Param$theta*deltaG), 1/sqrt(deltaG))
    L0 = dbind(gplvm$Param$LD, backsolve(chol(gplvm$Param$K),diag(M)) * sqrt(gplvm$Param$theta))
...
LF=10
    ...
    function(Ta){
        ...
        grp = cutree(hclust(dist(Tall)),h=0.5)
    ...
    Ks = updateKernelSE(gplvm$Param$Xi[[2]][,-c(1,3)],reduceDim1(gplvm$Param$Ta[[2]][,-c(1,3)]),gplvm$Param$rho[[2]][-c(1,3)]*rhoscale)
    U = Ks$Knm
